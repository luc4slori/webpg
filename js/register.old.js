...
var URI = {};
	...
	URI.POST_COMENTARIOS = "actions/api-comentario.php?action=guardar";
	URI.POST_LOGOUT = "actions/api-login.php?action=logout";  
	URI.POST_USERS = "actions/api-user.php?action=guardar";
	URI.POST_SOCIAL = "actions/api-user.php?action=logSocial";
	URI.POST_LOGIN =  "actions/api-login.php?action=login";
	URI.GET_COMENTARIOS = "actions/api-comentario.php?action=listarPorIdNota";
	URI.TEMPLATE_COMENTARIOS = "templates/comentarios.html";
	URI.LOG_TWT = "actions/api-twt.php?action=login";
	URI.SET_CURRENT_URL = "actions/api-server.php?action=setCurrentURL";
	URI.GET_TIPO_USER = "actions/api-server.php?action=getTipoUser";
...
var mostrarPagLog = function(sessions){
	...
	if (urlActual.indexOf("user-nota") !=-1) {
	...
	if (sessions['belong']==''){
		$('#btn-account-nav').children('img').attr('src','img/account-grey.jpg');
	} else {
		$('#btn-account-nav').children('img').attr('src', sessions['imagenLocal']);
	}
...
/***********************************************************************/	
	...
	$('.btn-twt').on('click', null, function(e) {
		...
		var urlActual =  window.location.href.split('id=')[0];
		urlActual = urlActual + 'id=';
		urlActual= urlActual + $('#nota_id_user_nota').val();
		e.preventDefault();
